/*********************************************************************************************
 * In this version, one has to revise N, dt, and Lf simultaneously for main.cpp and MPC.cpp, *
 * when we tuning parameters. Next work is to revise the code and offer a union interface to *
 * adjust parameters like N, dt, Lf, c1, c2, ..., c10 etc. The possible method is to set the *
 * FG_eval class to be a friend class of MPC.                                                *
 *********************************************************************************************/
...
int main() {
  ...
  // Timesteps length
  int N = 10;
  // Note. if you want to tune Lf and N, please also change the corresponding values in
  // MPC.cpp
    
  // Set a variable to save the previous time stamp.
  // This will be used to estimate the latency
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double psi = j[1]["psi"];
          double v = j[1]["speed"];
          //std::cout << "speed: " << v << std::endl; // one can find the speed is in mph.
          ...
          // Calculate the real latency of the last round of optimization
          ...
          // Fit for the reference line with a third polynomial
          ...
          // We ignore the unexpected latency (> 0.25s)
